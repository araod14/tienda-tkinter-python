...
class Tienda():
    ...
    def __init__(self, ventana_producto):
        ...
        ventana_producto.title('Tienda de Productos')
        ventana_producto.geometry('770x670')
        ...
        #Acciones de menu
        self.boton_registrar=Productos.add_command(label="Registrar",image=self.img_registrar,compound=LEFT)
        self.boton_buscar=Productos.add_command(label="Buscar",image=self.img_buscar,compound=LEFT)
        self.boton_nueva_venta=Ventas.add_command(label="Nueva Venta",image=self.img_nueva_venta,compound=LEFT)        
        self.boton_ventas=Ventas.add_command(label="Ventas",image=self.img_ventas,compound=LEFT)        
        self.boton_cliente=Ventas.add_command(label="Clientes",image=self.img_cliente,compound=LEFT)        
        self.boton_informacion=Informacion.add_command(label="Codigo Producto",image=self.img_codigo,compound=LEFT)
        self.boton_informacion=Informacion.add_command(label="Informacion del sistema",image=self.img_informacion,compound=LEFT)
    ...
    def widgets_crud(self):
        ...
        #Logo 
        imagen=Image.open("./img/descarga (1).png")
    ...
    def Actualizar(self,nuevo_codigo,nuevo_nombre,nuevo_combo_categoria,nueva_cantidad,nuevo_precio,nueva_descripcion,codigo,nombre):
        query='UPDATE producto SET codigo = ?, nombre = ?, categoria = ?, cantidad =?, precio=?, descripcion =? WHERE codigo = ? AND nombre =?'
